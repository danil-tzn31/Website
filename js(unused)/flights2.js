// ============================================
// FLIGHT SEARCH SCRIPT (Google Flights API - Final Fix)
// ============================================

(function() {

    console.log('Google Flights script loaded - Version 3.0');

    // API Config
    const G_API_KEY = '5559c078d8mshd05aafeafe9af7ap11c3d1jsn9352a8e5b475'; 
    ...
    if (flightSearchBtn) {
        ...
        flightSearchBtn.addEventListener('click', async function(e) {
            ...
            // 3. STORE THE CODES (Crucial Step)
            currentFromCode = fromIata;
            currentToCode = toIata;
            
            console.log(`Searching: ${currentFromCode} -> ${currentToCode}`);
        ...
        try {
            ...
            if (result.status && result.data && result.data.itineraries) {
                ...
                if (allFlights.length > 0) {
                    ...
                    // Ensure codes are saved in case this function was called directly
    ...
    function renderFlightCards(flights, fromCode, toCode) {
        ...
        // --- SAFETY CHECK: If codes are missing, grab them from inputs directly ---
        // This prevents the "---" issue if the state was lost
        let displayFrom = fromCode;
        let displayTo = toCode;

        if (!displayFrom || displayFrom === '---') {
             const rawFrom = document.getElementById('flight-from').value.trim().toLowerCase();
             displayFrom = LOCAL_AIRPORTS[rawFrom] || rawFrom.toUpperCase() || 'Origin';
        }

        if (!displayTo || displayTo === '---') {
             const rawTo = document.getElementById('flight-to').value.trim().toLowerCase();
